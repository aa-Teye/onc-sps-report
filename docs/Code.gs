...
function doPost(e) {
  try {
    ...
    if (action === 'deleteShepherd')          return deleteShepherd(data);
    if (action === 'promoteSoulToRoster')     return promoteSoulToRoster(data);
    if (action === 'assignFirstTimerToShepherd') return assignFirstTimerToShepherd(data);
    if (action === 'seedDatabaseRoster')      return seedDatabaseRoster(data);
...
function doGet(e) {
  ...
  try {
    ...
    else if (action === 'promoteSoulToRoster') {
      response = promoteSoulToRoster({
        soulId: e.parameter.soulId,
        targetShepherd: e.parameter.targetShepherd,
        stream: e.parameter.stream,
        zone: e.parameter.zone,
        promotedBy: e.parameter.promotedBy
      });
    }
    else if (action === 'assignFirstTimerToShepherd') {
      response = assignFirstTimerToShepherd({
        guestId: e.parameter.guestId,
        targetShepherd: e.parameter.targetShepherd,
        stream: e.parameter.stream,
        zone: e.parameter.zone,
        assignedBy: e.parameter.assignedBy
      });
    }
...
// 3: Foundation School
// 4: Member Now
// Standalone from Seekers/DiscipleshipJourney. Once a soul has reached
// Converted or Foundation School, progress can also sync into DiscipleshipJourney.
// Soul entries are NOT added to SPS or Microchurch rosters until explicitly
// promoted via promoteSoulToRoster.

var SOUL_TO_JOURNEY_STAGE_MAP = { 2: 1, 3: 2, 4: 6 };
...
function deleteShepherd(data) {
  ...
  var existing = records.find(function (r) {
    return r.Name && r.Name.toString().trim().toLowerCase() === name.toLowerCase() &&
           (!type || r.Type.toString().trim().toUpperCase() === type);
  });

  if (existing) {
    sheet.deleteRow(existing._row);
  } else {
    // If not in Shepherds sheet yet (legacy static entry), add tombstone with Status='Deleted' or Zone='DELETED'
    var now = new Date();
    var newRow = sheet.getLastRow() + 1;
    sheet.getRange(newRow, 3).setNumberFormat('@');
    sheet.getRange(newRow, 6).setNumberFormat('@');
    sheet.getRange(newRow, 1, 1, 8).setValues([[
      'SHP-DEL-' + now.getTime(), name, '', type || 'SPS', 'DELETED', '0000',
      deletedBy, formatDate(now)
    ]]);
  }

  logAudit(SpreadsheetApp.getActiveSpreadsheet(), 'SHEPHERD_DELETED', deletedBy, name + ' (' + (type || 'All') + ')');
  return jsonResponse({ status: 'success', message: 'Shepherd deleted successfully' });
}

// ── Promote a Soul Tracker prospect to the official Members roster ──────────
function promoteSoulToRoster(data) {
  var soulId = data.soulId;
  var targetShepherd = (data.targetShepherd || '').toString().trim();
  var stream = (data.stream || 'sps').toString().toLowerCase();
  var zone = (data.zone || '').toString().trim();
  var promotedBy = data.promotedBy || data.requestingRole || 'Shepherd';

  if (!soulId) return jsonResponse({ status: 'error', message: 'Missing soulId' });
  if (!targetShepherd) return jsonResponse({ status: 'error', message: 'Missing target shepherd' });

  var soulSheet = ensureSheet('SoulTracker', ['SoulID', 'Name', 'Phone', 'Notes', 'ShepherdType', 'ShepherdName', 'CurrentStage', 'StageEntryDate', 'DateAdded', 'LastUpdated', 'UpdatedBy']);
  var soulRecords = getSheetRecords(soulSheet);
  var soul = soulRecords.find(function (r) { return String(r.SoulID) === String(soulId); });
  if (!soul) return jsonResponse({ status: 'error', message: 'Soul not found' });

  // Update soul stage to 4 (Member Now)
  sheet = ensureSheet('SoulTracker', ['SoulID', 'Name', 'Phone', 'Notes', 'ShepherdType', 'ShepherdName', 'CurrentStage', 'StageEntryDate', 'DateAdded', 'LastUpdated', 'UpdatedBy']);
  var nowIso = formatISODate(new Date());
  soulSheet.getRange(soul._row, 7).setValue(4);
  soulSheet.getRange(soul._row, 8).setValue(nowIso);
  soulSheet.getRange(soul._row, 10).setValue(nowIso);
  soulSheet.getRange(soul._row, 11).setValue(promotedBy);

  // Add into Members sheet
  var membersSheet = ensureSheet(MEMBERS_SHEET, MEMBERS_HEADERS);
  var memRecords = getSheetRecords(membersSheet);
  var dup = memRecords.find(function (r) {
    return r.Name && r.Name.toString().trim().toLowerCase() === soul.Name.trim().toLowerCase() &&
           r.Shepherd && r.Shepherd.toString().trim().toLowerCase() === targetShepherd.toLowerCase();
  });

  if (dup) {
    // Already in roster; just update status to Member and ensure correct shepherd
    membersSheet.getRange(dup._row, 4).setValue(targetShepherd);
    membersSheet.getRange(dup._row, 6).setValue(stream);
    membersSheet.getRange(dup._row, 8).setValue('Member');
    if (zone) membersSheet.getRange(dup._row, 5).setValue(zone);
    logAudit(SpreadsheetApp.getActiveSpreadsheet(), 'SOUL_PROMOTED', promotedBy, soul.Name + ' -> ' + targetShepherd + ' (' + stream.toUpperCase() + ')');
    return jsonResponse({ status: 'success', memberId: dup.MemberID });
  }

  var now = new Date();
  var newMemberId = 'MEM-' + now.getTime() + '-' + Math.floor(Math.random() * 1000);
  var newRow = membersSheet.getLastRow() + 1;
  membersSheet.getRange(newRow, 3).setNumberFormat('@');
  membersSheet.getRange(newRow, 7).setNumberFormat('@');
  membersSheet.getRange(newRow, 1, 1, 18).setValues([[
    newMemberId, soul.Name, soul.Phone || '', targetShepherd, zone, stream,
    formatDate(now), 'Member', 'Soul Tracker Promotion', promotedBy, formatDate(now), promotedBy, formatDate(now),
    '', '', '', '', soul.Notes || ''
  ]]);

  logAudit(SpreadsheetApp.getActiveSpreadsheet(), 'SOUL_PROMOTED', promotedBy, soul.Name + ' -> ' + targetShepherd + ' (' + stream.toUpperCase() + ')');
  return jsonResponse({ status: 'success', memberId: newMemberId });
}

// ── Recommend & Assign a First Timer to SPS or Microchurch Shepherd ────────
function assignFirstTimerToShepherd(data) {
  var guestId = data.guestId;
  var targetShepherd = (data.targetShepherd || '').toString().trim();
  var stream = (data.stream || 'sps').toString().toLowerCase();
  var zone = (data.zone || '').toString().trim();
  var assignedBy = data.assignedBy || 'First Timers Unit';

  if (!guestId) return jsonResponse({ status: 'error', message: 'Missing guestId' });
  if (!targetShepherd) return jsonResponse({ status: 'error', message: 'Missing target shepherd' });

  var ftSheet = ensureSheet(FIRST_TIMERS_SHEET, []);
  if (!ftSheet || ftSheet.getLastRow() <= 1) return jsonResponse({ status: 'error', message: 'First Timers sheet empty' });

  var ids = ftSheet.getRange(2, 1, ftSheet.getLastRow() - 1, 1).getValues();
  var targetRow = -1;
  for (var i = 0; i < ids.length; i++) {
    if (String(ids[i][0]) === String(guestId)) { targetRow = i + 2; break; }
  }
  if (targetRow === -1) return jsonResponse({ status: 'error', message: 'Guest record not found' });

  var guestValues = ftSheet.getRange(targetRow, 1, 1, 30).getValues()[0];
  var guestName = guestValues[6] || (guestValues[3] + ' ' + guestValues[5]);
  var guestPhone = guestValues[7] || '';
  var guestResidence = guestValues[14] || '';

  // Update First Timers row follow-up status and invitedBy/shepherd note
  ftSheet.getRange(targetRow, 24).setValue('Assigned to ' + targetShepherd + ' (' + stream.toUpperCase() + ')');

  // Insert into Members sheet as Seeker/Member under that Shepherd
  var membersSheet = ensureSheet(MEMBERS_SHEET, MEMBERS_HEADERS);
  var memRecords = getSheetRecords(membersSheet);
  var dup = memRecords.find(function (r) {
    return r.Name && r.Name.toString().trim().toLowerCase() === guestName.trim().toLowerCase() &&
           r.Shepherd && r.Shepherd.toString().trim().toLowerCase() === targetShepherd.toLowerCase();
  });
  ...
  if (dup) {
    ...
    membersSheet.getRange(dup._row, 4).setValue(targetShepherd);
    membersSheet.getRange(dup._row, 5).setValue(zone || guestResidence);
    membersSheet.getRange(dup._row, 6).setValue(stream);
  } else {
    var now = new Date();
    memberId = 'MEM-FT-' + now.getTime();
    var newRow = membersSheet.getLastRow() + 1;
    membersSheet.getRange(newRow, 3).setNumberFormat('@');
    membersSheet.getRange(newRow, 7).setNumberFormat('@');
    membersSheet.getRange(newRow, 1, 1, 18).setValues([[
      memberId, guestName, guestPhone, targetShepherd, zone || guestResidence, stream,
      formatDate(now), 'Seeker', 'First Timers Recommendation', assignedBy, formatDate(now), '', '',
      guestValues[9] || '', guestValues[19] || '', guestValues[20] || '', guestValues[12] || '', guestResidence
    ]]);
  }

  logAudit(SpreadsheetApp.getActiveSpreadsheet(), 'FIRST_TIMER_RECOMMENDED', assignedBy, guestName + ' -> ' + targetShepherd + ' (' + stream.toUpperCase() + ')');
  return jsonResponse({ status: 'success', guestName: guestName, shepherd: targetShepherd, stream: stream, memberId: memberId });
}

// ── One-Time or On-Demand Seeder: Migrates all Shepherds & Members into database ──
function seedDatabaseRoster(data) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var shepherdsSheet = ensureSheet(SHEPHERDS_SHEET, SHEPHERDS_HEADERS);
  var membersSheet = ensureSheet(MEMBERS_SHEET, MEMBERS_HEADERS);

  var existingShepherds = {};
  getSheetRecords(shepherdsSheet).forEach(function (s) {
    if (s.Name) existingShepherds[s.Name.trim().toLowerCase() + '|' + (s.Type || '').trim().toUpperCase()] = true;
  });

  var existingMembers = {};
  getSheetRecords(membersSheet).forEach(function (m) {
    if (m.Name) existingMembers[m.Name.trim().toLowerCase() + '|' + (m.Shepherd || '').trim().toLowerCase()] = true;
  });

  var shepherdsAdded = 0;
  var membersAdded = 0;
  var now = new Date();

  // Incoming data passed from payload or default roster
  var spsShepherds = (data && data.spsShepherds) ? data.spsShepherds : [];
  var mcShepherds = (data && data.mcShepherds) ? data.mcShepherds : [];

  spsShepherds.forEach(function (s) {
    var sKey = s.name.trim().toLowerCase() + '|SPS';
    if (!existingShepherds[sKey]) {
      var nextRow = shepherdsSheet.getLastRow() + 1;
      shepherdsSheet.getRange(nextRow, 3).setNumberFormat('@');
      shepherdsSheet.getRange(nextRow, 6).setNumberFormat('@');
      shepherdsSheet.getRange(nextRow, 1, 1, 8).setValues([[
        'SHP-' + now.getTime() + '-' + Math.floor(Math.random() * 1000),
        s.name, s.contact || '', 'SPS', s.zone || '', '0000',
        'Database Seeder', formatDate(now)
      ]]);
      existingShepherds[sKey] = true;
      shepherdsAdded++;
    }

    (s.members || []).forEach(function (m) {
      var mName = typeof m === 'string' ? m : m.name;
      var mContact = typeof m === 'object' ? (m.contact || '') : '';
      if (!mName) return;
      var mKey = mName.trim().toLowerCase() + '|' + s.name.trim().toLowerCase();
      if (!existingMembers[mKey]) {
        var mRow = membersSheet.getLastRow() + 1;
        membersSheet.getRange(mRow, 3).setNumberFormat('@');
        membersSheet.getRange(mRow, 7).setNumberFormat('@');
        membersSheet.getRange(mRow, 1, 1, 13).setValues([[
          'MEM-' + now.getTime() + '-' + Math.floor(Math.random() * 10000),
          mName, mContact, s.name, s.zone || '', 'sps',
          formatDate(now), 'Member', 'Roster Migration', 'Admin', formatDate(now), '', ''
        ]]);
  ...
  mcShepherds.forEach(function (s) {
    var sKey = s.name.trim().toLowerCase() + '|MC';
    if (!existingShepherds[sKey]) {
      var nextRow = shepherdsSheet.getLastRow() + 1;
      shepherdsSheet.getRange(nextRow, 3).setNumberFormat('@');
      shepherdsSheet.getRange(nextRow, 6).setNumberFormat('@');
      shepherdsSheet.getRange(nextRow, 1, 1, 8).setValues([[
        'SHP-' + now.getTime() + '-' + Math.floor(Math.random() * 1000),
        s.name, s.contact || '', 'MC', s.microchurch || '', '0000',
        'Database Seeder', formatDate(now)
      ]]);
      existingShepherds[sKey] = true;
      shepherdsAdded++;
    ...
    (s.members || []).forEach(function (m) {
      ...
      if (!existingMembers[mKey]) {
        ...
        membersSheet.getRange(mRow, 1, 1, 13).setValues([[
          'MEM-' + now.getTime() + '-' + Math.floor(Math.random() * 10000),
          mName, mContact, s.name, s.microchurch || '', 'mc',
          formatDate(now), 'Member', 'Roster Migration', 'Admin', formatDate(now), '', ''
        ]]);
        existingMembers[mKey] = true;
        membersAdded++;
  ...
  logAudit(ss, 'DATABASE_SEEDED', 'Admin', shepherdsAdded + ' shepherds, ' + membersAdded + ' members migrated to Sheets database');
  return jsonResponse({ status: 'success', shepherdsAdded: shepherdsAdded, membersAdded: membersAdded });
}
// SHARED HELPERS
// ============================================================
